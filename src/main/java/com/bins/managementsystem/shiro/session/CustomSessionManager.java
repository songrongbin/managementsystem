package com.bins.managementsystem.shiro.session;
...
public class CustomSessionManager {
     ...
     * session status
     */
    public static final String SESSION_STATUS = "bins-online-status";
    ...
    private UserOnlineDto getSessionBo(Session session) {
        ...
        if (obj instanceof SimplePrincipalCollection) {
            SimplePrincipalCollection spc = (SimplePrincipalCollection) obj;
            /**
             * 获取用户登录的，@link UserRealm.doGetAuthenticationInfo(...)方法中
             * return new SimpleAuthenticationInfo(user,user.getPswd(), getName());的user 对象。
             */
            obj = spc.getPrimaryPrincipal();
            if (null != obj && obj instanceof User) {
                //存储session + user 综合信息
                UserOnlineDto userBo = new UserOnlineDto((User) obj);
                //最后一次和系统交互的时间
